...
def processar_vendas(df, df_codigos_barra):
    ...
    df['VALOR_LIQUIDO'] = np.where(
        df['QTDE_CANCELADA'] > 0, 
        0, 
        (df['PRECO_LIQUIDO'] * df['QTDE']) - df['DESCONTO_VENDA']
    )
    
    df.drop(columns=COLS_REMOVER['vendas'], inplace=True, errors='ignore')
    
    # Reordena colunas (valor líquido após qtde)
    ...
    idx_qtde = cols.index('QTDE') + 1
    df = df[cols[:idx_qtde] + ['VALOR_LIQUIDO'] + cols[idx_qtde:] + ['DESCONTO_VENDA']]
...
def processar_ecommerce(df):
    ...
    # CORREÇÃO: Remover duplicatas causadas por possíveis múltiplos JOINs
    # Mantém apenas uma linha por NF_SAIDA + SERIE_NF + ITEM (chave única do item)
    # Nota: A query SQL foi corrigida para evitar duplicatas na origem via subconsulta,
    # mas mantemos este tratamento como segurança adicional
    registros_antes = len(df)
    df = df.drop_duplicates(subset=['NF_SAIDA', 'SERIE_NF', 'ITEM'], keep='first')
    registros_depois = len(df)
    
    if registros_antes != registros_depois:
        print(f"  ⚠ Removidas {registros_antes - registros_depois:,} duplicatas (se houver duplicatas aqui, verifique a query SQL)")
    ...
    try:
        ...
        JOIN W_FATURAMENTO_PROD_02 fp WITH(NOLOCK) 
            ON f.FILIAL = fp.FILIAL AND f.NF_SAIDA = fp.NF_SAIDA AND f.SERIE_NF = fp.SERIE_NF
                ...
                -- CORREÇÃO: LEFT JOIN com subconsulta para evitar duplicatas de CLIENTES_VAREJO
                -- Usa ROW_NUMBER para pegar apenas um registro por cliente (priorizando UF não nula)
                LEFT JOIN (
                    SELECT 
                        CLIENTE_VAREJO,
                        UF
                    FROM (
                        SELECT 
                            CLIENTE_VAREJO,
                            UF,
                            ROW_NUMBER() OVER (PARTITION BY CLIENTE_VAREJO ORDER BY 
                                CASE WHEN UF IS NOT NULL THEN 0 ELSE 1 END,
                                CLIENTE_VAREJO) AS rn
