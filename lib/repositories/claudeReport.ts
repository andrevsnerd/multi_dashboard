      ...
      ? withRequest(async (request) => {
          ...
          const query = `
            WITH vendas_cte AS (
              ...
              ) cb ON RTRIM(LTRIM(cb.PRODUTO)) = RTRIM(LTRIM(vp.PRODUTO))
                 AND (RTRIM(LTRIM(CAST(cb.COR_PRODUTO AS VARCHAR(20)))) = RTRIM(LTRIM(CAST(vp.COR_PRODUTO AS VARCHAR(20)))) OR TRY_CONVERT(INT, cb.COR_PRODUTO) = TRY_CONVERT(INT, vp.COR_PRODUTO))
              LEFT JOIN (
            ...
            trocas_cte AS (
              ...
              LEFT JOIN (
                ...
                FROM PRODUTO_CORES WITH (NOLOCK)
                GROUP BY PRODUTO, COR_PRODUTO
              ) cb ON RTRIM(LTRIM(cb.PRODUTO)) = RTRIM(LTRIM(vt.PRODUTO))
                 AND (RTRIM(LTRIM(CAST(cb.COR_PRODUTO AS VARCHAR(20)))) = RTRIM(LTRIM(CAST(vt.COR_PRODUTO AS VARCHAR(20)))) OR TRY_CONVERT(INT, cb.COR_PRODUTO) = TRY_CONVERT(INT, vt.COR_PRODUTO))
              LEFT JOIN (
                SELECT pb.PRODUTO, ISNULL(pb.COR_PRODUTO, '') AS COR_PRODUTO, MIN(pb.CODIGO_BARRA) AS CODIGO_BARRA
                FROM PRODUTOS_BARRA pb WITH (NOLOCK)
                WHERE ISNULL(pb.CODIGO_BARRA, '') <> ''
