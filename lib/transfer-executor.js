...
// ---------------------------------------------------------------------------
// Camada A — idempotência em memória (best-effort). Dedupe reenvios rápidos que
// carregam a MESMA idempotencyKey (retry automático, duplo-clique, retomada de
// rede). Devolve o resultado já calculado sem gerar um segundo romaneio. Não é
// durável (some se o processo reiniciar); a proteção durável é a Camada B, que
// consulta o próprio banco. Juntas cobrem todos os casos observados.
// ---------------------------------------------------------------------------
const _idemCache = new Map(); // idempotencyKey -> { result, ts }
const _IDEM_TTL_MS = 10 * 60 * 1000;
function _idemGet(key) {
  if (!key) return null;
  const hit = _idemCache.get(key);
  if (!hit) return null;
  if (Date.now() - hit.ts > _IDEM_TTL_MS) {
...
function _idemSet(key, result) {
  if (!key) return;
  _idemCache.set(key, { result, ts: Date.now() });
  if (_idemCache.size > 500) {
    const now = Date.now();
    for (const [k, v] of _idemCache) {
      if (now - v.ts > _IDEM_TTL_MS) _idemCache.delete(k);
    }
  }
}
 ...
 * @param {string|null} [params.idempotencyKey] Chave de dedupe (Camada A). Mesma chave = não gera 2º romaneio.
 * @param {boolean} [params.permitirDuplicado] Se true, ignora a trava de transferência idêntica recente (Camada B).
 * @returns {Promise<{ success: boolean, romaneioSaida: string, romaneioEntrada: string, message: string, deduplicado?: boolean }>}
 */
async function executeTransfer(pool, params) {
  const {
    ...
    observacao = null,
    idempotencyKey = null,
    // Quando o operador confirma que quer MESMO repetir uma transferência idêntica
    // recente (ex.: enviar uma segunda peça igual), o cliente reenvia com este flag
    // e a trava anti-duplicação (Camada B) é ignorada nesta chamada.
    permitirDuplicado = false,
  } = params;

  // Camada A: se esta mesma chave já foi executada há pouco, devolve o mesmo
  // romaneio em vez de criar outro. Blinda retries/reenvios de rede.
  const cached = _idemGet(idempotencyKey);
  if (cached) {
    return { ...cached, deduplicado: true, message: `${cached.message} (reenvio ignorado)` };
  }
  ...
  // ---------------------------------------------------------------------------
  // Camada B — trava anti-duplicação durável (consulta o próprio banco).
  // Antes de gerar um novo romaneio, procura uma saída IDÊNTICA recente
  // (mesma origem+destino+responsável+produto+cor+qtde) numa janela curta.
  // Cobre os dois padrões observados: duplo-scan (romaneios consecutivos, ~7s)
  // e "achou que falhou e refez" (mesmo item minutos depois). O destino faz
  // parte da chave de propósito — a Oscar distribui o MESMO produto para lojas
  // diferentes, e isso é legítimo. Se o operador quiser mesmo repetir uma
  // transferência idêntica, o cliente reenvia com permitirDuplicado=true.
  // ---------------------------------------------------------------------------
  if (!permitirDuplicado) {
    const JANELA_MIN = 15;
    let dupEncontrada = null;
    // Fail-open: se a CONSULTA da trava falhar por qualquer motivo, apenas logamos
    // e deixamos a transferência seguir normalmente. A trava é só uma proteção
    // extra — nunca pode impedir uma saída/entrada legítima de acontecer.
    try {
      const reqDup = pool.request();
      reqDup.input('filialOrigemNome', filialOrigemNome);
      reqDup.input('filialDestinoNome', filialDestinoNome);
      reqDup.input('responsavel', responsavel || ' ');
      reqDup.input('produto', produto.trim());
      reqDup.input('cor', corProduto ? corProduto.trim() : '');
      reqDup.input('qtde', qtdeSaida);
      const resultDup = await reqDup.query(`
        SELECT TOP 1
          LTRIM(RTRIM(s.ROMANEIO_PRODUTO)) AS ROMANEIO,
          CONVERT(VARCHAR(19), s.EMISSAO, 120) AS EMISSAO,
          DATEDIFF(SECOND, s.EMISSAO, GETDATE()) AS SEG_ATRAS
        ...
        WHERE s.EMISSAO >= DATEADD(MINUTE, -${JANELA_MIN}, GETDATE())
          AND LTRIM(RTRIM(ISNULL(s.FILIAL, ''))) = @filialOrigemNome
          AND LTRIM(RTRIM(ISNULL(s.FILIAL_DESTINO, ''))) = @filialDestinoNome
          AND LTRIM(RTRIM(ISNULL(s.RESPONSAVEL, ''))) = @responsavel
          AND LTRIM(RTRIM(ISNULL(i.PRODUTO, ''))) = @produto
          AND LTRIM(RTRIM(ISNULL(i.COR_PRODUTO, ''))) = @cor
          AND i.QTDE = @qtde
        ORDER BY s.EMISSAO DESC
      `);
      if (resultDup.recordset.length > 0) dupEncontrada = resultDup.recordset[0];
    } catch (guardErr) {
      console.error('[Transfer] Trava anti-duplicação não pôde ser verificada (seguindo normalmente):', guardErr && guardErr.message);
      dupEncontrada = null;
    }

    if (dupEncontrada) {
      const seg = dupEncontrada.SEG_ATRAS != null ? Number(dupEncontrada.SEG_ATRAS) : 0;
      const err = new Error(
        `TRANSFERENCIA_DUPLICADA: já existe uma transferência idêntica (romaneio ${String(dupEncontrada.ROMANEIO).trim()}) ` +
        `para ${filialDestinoNome} feita há ${seg}s. Se deseja mesmo enviar outra igual, confirme o reenvio.`
      );
      err.code = 'TRANSFERENCIA_DUPLICADA';
      err.romaneioExistente = String(dupEncontrada.ROMANEIO).trim();
      err.segundosAtras = seg;
      throw err;
    }
  }
  ...
  // Camada A: registra o resultado para dedupe de reenvios com a mesma chave.
  _idemSet(idempotencyKey, resultadoFinal);
