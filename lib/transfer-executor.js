...
async function executeTransfer(pool, params) {
  ...
  // VALIDAÇÃO FINAL: Romaneio não deve existir
  ...
  if (resultValida.recordset[0]?.TOTAL > 0) {
    throw new Error(`Romaneio de saida ${romaneioSaida} ja existe para a filial ${filialOrigem}. Por favor, gere um novo romaneio.`);
  }

  // 2. Buscar nomes das filiais
  ...
  const resultNomesFiliais = await req2_nomes.query(`
  ...
  // Determinar CM_OPERACAO (empresas)
  ...
  for (const row of resultEmpresas.recordset) {
    const codFilial = row.COD_FILIAL?.toString().trim() || '';
    if (codFilial === filialOrigem.trim()) empresaOrigem = row.EMPRESA ? parseInt(String(row.EMPRESA)) : null;
    else if (codFilial === filialDestino.trim()) empresaDestino = row.EMPRESA ? parseInt(String(row.EMPRESA)) : null;
  }

  if (empresaOrigem === null) {
    const reqEmp = pool.request();
    reqEmp.input('filialOrigem', sql.VarChar, filialOrigem);
    const r = await reqEmp.query(`SELECT EMPRESA FROM FILIAIS WITH (NOLOCK) WHERE COD_FILIAL = @filialOrigem`);
    empresaOrigem = r.recordset.length > 0 && r.recordset[0].EMPRESA ? parseInt(String(r.recordset[0].EMPRESA)) : 8;
  }
  if (empresaDestino === null) {
    const reqEmp = pool.request();
    reqEmp.input('filialDestino', sql.VarChar, filialDestino);
    const r = await reqEmp.query(`SELECT EMPRESA FROM FILIAIS WITH (NOLOCK) WHERE COD_FILIAL = @filialDestino`);
  ...
  if (corExata) req14_verif.input('corExata', sql.VarChar, corExata);
  ...
  if (corExata) req14.input('corExata', sql.VarChar, corExata);
  ...
  if (corExata) req14_depois.input('corExata', sql.VarChar, corExata);
  ...
  if (estoqueDepoisUpdate === null || estoqueDepoisUpdate !== estoqueAntesUpdate - qtdeSaida) {
    if (estoqueDepoisUpdate === null || estoqueDepoisUpdate === estoqueAntesUpdate) {
      ...
      if (corExata) req14_alt.input('corExata', sql.VarChar, corExata);
      ...
      if (corExata) req14_verif_alt.input('corExata', sql.VarChar, corExata);
  ...
  if (corProdutoParaQuery) req15.input('corProdutoParaQuery', sql.VarChar, corProdutoParaQuery);
  ...
  if (existeEstoque) {
    ...
    if (corDestinoExata) req16.input('corDestinoExata', sql.VarChar, corDestinoExata);
    ...
    if (corDestinoExata) req16_depois.input('corDestinoExata', sql.VarChar, corDestinoExata);
    ...
    if (estoqueDestinoDepois === null || estoqueDestinoDepois !== estoqueDestinoAntesUpdate + qtdeEntrada) {
      if (estoqueDestinoDepois === null || estoqueDestinoDepois === estoqueDestinoAntesUpdate) {
        ...
        if (corDestinoExata) req16_alt.input('corDestinoExata', sql.VarChar, corDestinoExata);
        ...
        if (corDestinoExata) req16_verif_alt.input('corDestinoExata', sql.VarChar, corDestinoExata);
