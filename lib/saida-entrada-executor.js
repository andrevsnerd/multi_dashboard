...
async function executeSaidaLote(pool, params) {
  ...
  // Gerar próximo romaneio de saída (uma vez)
  const req1 = pool.request();
  const resultRomaneio = await req1.query(`
    SELECT TOP 1 ROMANEIO_PRODUTO FROM ESTOQUE_PROD_SAI WITH (NOLOCK)
  ...
  if (resultRomaneio.recordset.length > 0) {
    const romaneioAtual = resultRomaneio.recordset[0].ROMANEIO_PRODUTO?.toString().trim() || '';
    if (romaneioAtual) romaneioSaida = `${parseInt(romaneioAtual) + 1}`.padStart(6, '0');
  }

  // Garantir unicidade do romaneio
  let tentativas = 0;
  while (tentativas < 10) {
    const reqV = pool.request();
    reqV.input('romaneioSaida', romaneioSaida);
    reqV.input('filialNome', filialNome);
    const resultV = await reqV.query(`
      SELECT COUNT(*) as TOTAL FROM ESTOQUE_PROD_SAI WITH (NOLOCK)
      WHERE LTRIM(RTRIM(ROMANEIO_PRODUTO)) = @romaneioSaida AND LTRIM(RTRIM(FILIAL)) = @filialNome
    `);
    if (resultV.recordset[0]?.TOTAL === 0) break;
    romaneioSaida = `${parseInt(romaneioSaida) + 1}`.padStart(6, '0');
    tentativas++;
  }
  ...
  if (observacaoEscaped) {
    ...
    await req5.query(`
      INSERT INTO LOJA_SAIDAS (
        ...
        @romaneioSaida, @filial, @dataStr, @responsavel,
        NULL, '', 0, @qtdeTotal, 0, 0, @dataStr, 1, 0, @observacao
      )
    `);
  } else {
    await req5.query(`
      INSERT INTO LOJA_SAIDAS (
        ...
        @romaneioSaida, @filial, @dataStr, @responsavel,
        NULL, '', 0, @qtdeTotal, 0, 0, @dataStr, 1, 0
      )
    `);
  }
...
async function executeEntradaLote(pool, params) {
  ...
  // Gerar próximo romaneio de entrada (uma vez)
  const req1 = pool.request();
  const resultRomaneio = await req1.query(`
    SELECT TOP 1 ROMANEIO_PRODUTO FROM ESTOQUE_PROD_ENT WITH (NOLOCK)
    WHERE ISNUMERIC(ROMANEIO_PRODUTO) = 1 AND LEN(LTRIM(RTRIM(ROMANEIO_PRODUTO))) = 6
    ORDER BY CAST(ROMANEIO_PRODUTO AS INT) DESC
  `);
  let romaneioEntrada = '016042';
  if (resultRomaneio.recordset.length > 0) {
    const romaneioAtual = resultRomaneio.recordset[0].ROMANEIO_PRODUTO?.toString().trim() || '';
    if (romaneioAtual) romaneioEntrada = `${parseInt(romaneioAtual) + 1}`.padStart(6, '0');
  }
  ...
  while (tentativas < 10) {
    const reqV = pool.request();
    reqV.input('romaneioEntrada', romaneioEntrada);
    reqV.input('filialNome', filialNome);
    const resultV = await reqV.query(`
      SELECT COUNT(*) as TOTAL FROM ESTOQUE_PROD_ENT WITH (NOLOCK)
      WHERE LTRIM(RTRIM(ROMANEIO_PRODUTO)) = @romaneioEntrada AND LTRIM(RTRIM(FILIAL)) = @filialNome
    `);
    if (resultV.recordset[0]?.TOTAL === 0) break;
    romaneioEntrada = `${parseInt(romaneioEntrada) + 1}`.padStart(6, '0');
    tentativas++;
  }
