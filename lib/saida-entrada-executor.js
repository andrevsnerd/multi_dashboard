...
async function executeSaidaLoteCore(pool, params) {
  ...
  /**
   * O DESTINO DESTA TELA E ROTULO DA DASHBOARD - NAO E TRANSITO DO ERP.
   *
   * A saida daqui e avulsa: nasce com CM_OPERACAO '011' e sem ROMANEIO_DESTINO,
   * ou seja, sem o par saida/entrada que o Linx exige numa transferencia real.
   * Gravar FILIAL_DESTINO nesse cenario faz o ERP tratar o romaneio como
   * transferencia, e ele SOME da tela 120003SPK (Saida de Produto Acabado do
   * Estoque) - foi o que tirou da listagem os romaneios de setembro/2026.
   *
   * Por isso a coluna vai NULL, igual ja fazem a saida de item unico (acima) e
   * `lib/transfer-executor.js` quando CM_OPERACAO e '011'. O destino nao se
   * perde: a rota grava em `destino-romaneio-store` (Neon), que e de onde as
   * telas de Romaneios e Controle de Transferencias leem.
   */
  const filialDestinoValor = null;
...
/**
 * TRAVA DO LINX — não se mexe em movimento de estoque anterior ao ajuste ou ao
 * fechamento.
 *
 * Os triggers LXI/LXU/LXD de ESTOQUE_PROD1_ENT e ESTOQUE_PROD1_SAI recusam
 * qualquer inclusão/alteração de item quando:
 *
 *  1. a EMISSAO do romaneio é <= CM_DATA_FECHAMENTO.DATA_SALDO_PA da filial
 *     (estoque fechado até aquela data), ou
 *  2. a EMISSAO do romaneio é < ESTOQUE_PRODUTOS.DATA_AJUSTE do item
 *     (aquele produto × cor × filial foi inventariado depois do romaneio).
 *
 * Quando isso acontece o trigger dá `raiserror` + `rollback transaction`, e o
 * que chega no driver é "The transaction ended in the trigger. The batch has
 * been aborted." — mensagem que não diz nada a quem está bipando peça. Pior:
 * via proxy o erro ainda passava pelo tradutor de rede e virava "o servidor
 * proxy não respondeu", mandando investigar o proxy no lugar do romaneio.
 *
 * Por isso a checagem é feita ANTES de escrever, com exatamente a mesma
 * condição do trigger, e o motivo volta em português. Foi o caso do romaneio
 * 028674 (NERD, emissão 05/01/2026): o inventário de 03/09/2026 fechou a porta
 * para 532 dos 617 itens dele.
 *
 * @param tipo  'entrada' → ESTOQUE_PROD_ENT/ESTOQUE_PROD1_ENT
 *              'saida'   → ESTOQUE_PROD_SAI/ESTOQUE_PROD1_SAI
 * @param itens lista `{ produto, corProduto }` a acrescentar. `null` verifica os
 *              itens que JÁ estão no romaneio (usado pela tela, antes de bipar).
 */
async function verificarTravaMovimento(pool, { tipo, romaneio, filial, itens = null }) {
  const romaneioTrim = (romaneio || '').trim();
  const filialTrim = (filial || '').trim();
  if (!romaneioTrim || !filialTrim) {
    return { emissao: null, fechamento: null, travados: [], itensVerificados: 0 };
  }

  const tabelaCab = tipo === 'saida' ? 'ESTOQUE_PROD_SAI' : 'ESTOQUE_PROD_ENT';
  const tabelaItens = tipo === 'saida' ? 'ESTOQUE_PROD1_SAI' : 'ESTOQUE_PROD1_ENT';

  // Origem dos pares produto × cor a checar: a lista que veio da tela ou os
  // itens do próprio romaneio.
  let fonteItens;
  if (itens === null) {
    fonteItens = `
        SELECT LTRIM(RTRIM(i.PRODUTO)) AS PRODUTO,
               ISNULL(LTRIM(RTRIM(CAST(i.COR_PRODUTO AS VARCHAR(20)))), '') AS COR
          FROM ${tabelaItens} i WITH (NOLOCK)
         WHERE LTRIM(RTRIM(i.ROMANEIO_PRODUTO)) = @romaneio
           AND LTRIM(RTRIM(i.FILIAL)) = @filial`;
  } else {
    const pares = itens
      .map((item) => {
        const produto = (item.produto || '').toString().trim().replace(/'/g, "''");
        const cor = (item.corProduto ?? item.cor ?? '').toString().trim().replace(/'/g, "''");
        return produto ? `('${produto}','${cor}')` : null;
      })
      .filter(Boolean);
    if (pares.length === 0) {
      return { emissao: null, fechamento: null, travados: [], itensVerificados: 0 };
    }
    // Literais (não parâmetros) porque é uma lista de tamanho variável; os
    // valores vêm do cadastro/da lista da tela e as aspas são escapadas.
    fonteItens = `SELECT PRODUTO, COR FROM (VALUES ${pares.join(',')}) v(PRODUTO, COR)`;
  }

  const req = pool.request();
  req.input('romaneio', romaneioTrim);
  req.input('filial', filialTrim);
  // Uma consulta só: o proxy devolve apenas o primeiro recordset, então
  // fechamento e itens travados voltam na mesma lista, separados por TIPO.
  const result = await req.query(`
    ;WITH cab AS (
      SELECT TOP 1 c.EMISSAO, c.FILIAL
        FROM ${tabelaCab} c WITH (NOLOCK)
       WHERE LTRIM(RTRIM(c.ROMANEIO_PRODUTO)) = @romaneio
         AND LTRIM(RTRIM(c.FILIAL)) = @filial
    ), itens AS (${fonteItens})
    SELECT
      'EMISSAO' AS TIPO, '' AS PRODUTO, '' AS COR,
      (SELECT CONVERT(VARCHAR(10), MIN(cab.EMISSAO), 103) FROM cab) AS DATA
    UNION ALL
    SELECT
      'FECHAMENTO', '', '',
      (SELECT CONVERT(VARCHAR(10), MAX(f.DATA_SALDO_PA), 103)
         FROM cab
         JOIN CM_DATA_FECHAMENTO f WITH (NOLOCK)
           ON LTRIM(RTRIM(f.FILIAL)) = LTRIM(RTRIM(cab.FILIAL))
        WHERE cab.EMISSAO <= f.DATA_SALDO_PA)
    UNION ALL
    SELECT
      'AJUSTE', itens.PRODUTO, itens.COR,
      CONVERT(VARCHAR(10), MAX(es.DATA_AJUSTE), 103)
      FROM itens
      CROSS JOIN cab
      JOIN ESTOQUE_PRODUTOS es WITH (NOLOCK)
        ON LTRIM(RTRIM(es.FILIAL)) = LTRIM(RTRIM(cab.FILIAL))
       AND LTRIM(RTRIM(es.PRODUTO)) = itens.PRODUTO
       AND (
         ISNULL(LTRIM(RTRIM(CAST(es.COR_PRODUTO AS VARCHAR(20)))), '') = itens.COR
         OR TRY_CONVERT(INT, es.COR_PRODUTO) = TRY_CONVERT(INT, itens.COR)
       )
     WHERE cab.EMISSAO < es.DATA_AJUSTE
     GROUP BY itens.PRODUTO, itens.COR
  `);

  const linhas = result.recordset || [];
  const valor = (t) => linhas.find((l) => l.TIPO === t)?.DATA?.toString().trim() || null;
  const travados = linhas
    .filter((l) => l.TIPO === 'AJUSTE' && l.DATA)
    .map((l) => ({
      produto: (l.PRODUTO ?? '').toString().trim(),
      cor: (l.COR ?? '').toString().trim(),
      data: (l.DATA ?? '').toString().trim(),
    }));
  ...
  return {
    ...
    travados,
    itensVerificados: itens === null ? linhas.filter((l) => l.TIPO === 'AJUSTE').length : itens.length,
  };
}

/**
 * Mensagem pronta para a tela quando a trava acima barra a operação — ou `null`
 * quando o romaneio aceita os itens.
 */
function motivoTravaMovimento(trava, romaneio) {
  if (!trava) return null;
  if (trava.fechamento) {
    return `Romaneio ${romaneio} é de ${trava.emissao || 'data anterior'} e o estoque desta filial está fechado até ${trava.fechamento} — o Linx não aceita mais lançar itens nele.`;
  }
  if (trava.travados.length > 0) {
    const amostra = trava.travados
      .slice(0, 3)
      .map((t) => `${t.produto}${t.cor ? ` cor ${t.cor}` : ''} (ajuste em ${t.data})`)
      .join(', ');
    const resto = trava.travados.length > 3 ? ` e outros ${trava.travados.length - 3}` : '';
    return `O estoque foi ajustado/inventariado depois da emissão deste romaneio (${trava.emissao || 's/ data'}): ${amostra}${resto}. O Linx bloqueia movimento anterior ao ajuste — lance esses itens em um romaneio novo.`;
  }
  return null;
}

/**
 * Aviso para a tela ANTES de bipar: os itens que já estão no romaneio caíram
 * atrás de um ajuste de inventário.
 *
 * Não bloqueia porque a trava do Linx é por produto × cor — um item que não
 * entrou no inventário ainda passa. Mas quando quase todo o romaneio está
 * travado (foi o caso do 028674: 532 de 617), o operador precisa saber disso
 * antes de bipar 600 peças para depois a gravação ser recusada.
 */
function avisoTravaMovimento(trava) {
  if (!trava || trava.travados.length === 0) return null;
  const ultimo = trava.travados
    .map((t) => t.data)
    .sort()
    .pop();
  return `${trava.travados.length} de ${trava.itensVerificados} produto(s) deste romaneio (emissão ${trava.emissao || 's/ data'}) foram ajustados/inventariados depois — o último em ${ultimo}. O Linx recusa acrescentar item anterior ao ajuste: só passam produtos que não entraram no inventário.`;
}

/**
 * Tipos das colunas de chave dos itens de romaneio, para o UPDATE cair em
 * SEEK na PK em vez de varrer a tabela.
 *
 * Por que isso importa: o proxy manda todo parâmetro string como NVARCHAR e o
 * banco é SQL_Latin1_General_CP1_CI_AS — comparar coluna CHAR com parâmetro
 * NVARCHAR nessa collation impede o seek. Somado ao `LTRIM(RTRIM(col)) = @p`
 * que estava no WHERE, o UPDATE varria as ~356 mil linhas de
 * ESTOQUE_PROD1_ENT pegando lock em cada uma — bastava o Linx estar com
 * qualquer linha da tabela travada para a gravação esperar sem limite (e o
 * pedido morrer no timeout de 5 min do proxy). Copiando o parâmetro para uma
 * variável local do tipo exato da coluna, o plano volta a ser um seek de 1 linha.
 */
function montarChaveItemTipada() {
  return `DECLARE @kRomaneio CHAR(8) = @keyRomaneio,
            @kFilial VARCHAR(25) = @keyFilial,
            @kProduto CHAR(12) = @keyProduto,
            @kCor CHAR(10) = @keyCor;`;
}

const MATCH_CHAVE_ITEM = `
      ROMANEIO_PRODUTO = @kRomaneio
      AND FILIAL = @kFilial
      AND PRODUTO = @kProduto
      AND COR_PRODUTO = @kCor`;

/**
 * Acha a linha do item no romaneio (se já existir) e o TAMANHO_BASE do produto
 * em uma única ida ao banco.
 *
 * A busca continua tolerante como no resto do arquivo (cor '06' vs '6', filial
 * com padding), mas o que volta são os valores EXATOS gravados na chave — é com
 * eles que o UPDATE endereça a linha, sem `LTRIM` no WHERE.
 */
async function localizarItemDoRomaneio(pool, { tabela, romaneio, filialGrav, produto, cor }) {
  const req = pool.request();
  ...
  const result = await req.query(`
    SELECT TOP 1
      ISNULL((SELECT TOP 1 ISNULL(p.TAMANHO_BASE, 1)
                FROM PRODUTOS p WITH (NOLOCK)
               WHERE p.PRODUTO = @produto), 1) AS TAMANHO_BASE,
      i.ROMANEIO_PRODUTO AS K_ROMANEIO,
      i.FILIAL           AS K_FILIAL,
      i.PRODUTO          AS K_PRODUTO,
      i.COR_PRODUTO      AS K_COR
    FROM (SELECT 1 AS X) d
    LEFT JOIN ${tabela} i WITH (NOLOCK)
      ON LTRIM(RTRIM(i.ROMANEIO_PRODUTO)) = @romaneio
  ...
  const base = parseInt(row.TAMANHO_BASE, 10);
  const chave = row.K_ROMANEIO
    ? {
        romaneio: row.K_ROMANEIO,
        filial: row.K_FILIAL,
        produto: row.K_PRODUTO,
        cor: row.K_COR,
      }
    : null;

  return { tamanhoBase: base >= 1 && base <= 48 ? base : 1, chave };
}

/**
 * Erro do banco → mensagem que serve para quem está na tela.
 *
 * Os triggers do Linx respondem com `raiserror` + `rollback`, e o driver
 * entrega só o efeito ("The transaction ended in the trigger...") — o motivo
 * real vem em `precedingErrors`. Aqui os dois são juntados.
 */
function traduzirErroLinx(error, contexto) {
  const partes = [];
  const principal = (error && error.message ? String(error.message) : '').trim();
  const precedentes = Array.isArray(error?.precedingErrors)
    ? error.precedingErrors.map((e) => String(e?.message || '').trim()).filter(Boolean)
    : [];

  for (const msg of precedentes) if (!partes.includes(msg)) partes.push(msg);

  // Via proxy os erros chegam concatenados em uma string ("efeito | motivo"),
  // e o efeito ("batch has been aborted") não interessa a quem está na tela.
  if (partes.length === 0 && principal.includes('|')) {
    for (const pedaco of principal.split('|').map((p) => p.trim())) {
      if (/não é possível|nao e possivel|impossível|impossivel|operação cancelada/i.test(pedaco)) {
        partes.push(pedaco);
      }
    }
  }

  const texto = [principal, ...precedentes].join(' ').toLowerCase();
  if (texto.includes('lock request time out')) {
    return new Error(
      `${contexto}: o romaneio está em uso (provavelmente aberto no Linx) e o banco não liberou o registro. Feche o romaneio no Linx e tente de novo.`
    );
  }
  if (texto.includes('anterior ao ajuste') || texto.includes('anterior a #')) {
    // Conexão direta: o motivo está em precedingErrors. Via proxy: vem
    // concatenado na própria mensagem (ver /query no proxy-server).
    const motivo = partes.length > 0 ? partes.join(' | ') : principal;
    return new Error(`${contexto}: ${motivo}`);
  }
  if (texto.includes('transaction ended in the trigger')) {
    // Só o EFEITO chegou — o motivo ficou em `precedingErrors`, que o proxy
    // antigo (antes de mandar esse campo) descartava. Em vez de mostrar
    // "batch aborted" e mais nada, aponta a causa quase certa: é sempre um
    // trigger LX* recusando, e nesta tela a recusa é a trava de movimento
    // anterior ao ajuste/fechamento de estoque.
    const motivo = partes.length > 0 ? partes.join(' | ') : null;
    return new Error(
      `${contexto}: o Linx recusou o movimento (trigger). ${
        motivo ||
        'Causa provável: movimento anterior ao ajuste de inventário ou ao fechamento de estoque da filial — confira DATA_AJUSTE do item e CM_DATA_FECHAMENTO.'
      }`
    );
  }
  if (partes.length > 0) return new Error(`${contexto}: ${partes.join(' | ')}`);
  return error instanceof Error ? error : new Error(`${contexto}: ${principal || 'erro desconhecido'}`);
}
...
async function executeEntradaAppendCore(pool, params) {
  ...
  // Trava do Linx (fechamento/ajuste) ANTES de escrever: o trigger recusaria
  // com `rollback` e a tela receberia um erro que não diz nada.
  ...
  const motivoTrava = motivoTravaMovimento(trava, romaneioTrim);
  if (motivoTrava) throw new Error(motivoTrava);
  ...
  for (const item of itens) {
    ...
    // são valores distintos), então a busca também tolera o equivalente
    // numérico — e devolve a chave exata da linha achada.
    const { tamanhoBase, chave } = await localizarItemDoRomaneio(pool, {
      tabela: 'ESTOQUE_PROD1_ENT',
      romaneio: romaneioTrim,
      filialGrav,
      produto,
      cor,
    });
    const enCol = 'EN_' + tamanhoBase;
    ...
    try {
      if (chave) {
        req.input('keyRomaneio', chave.romaneio);
        req.input('keyFilial', chave.filial);
        req.input('keyProduto', chave.produto);
        req.input('keyCor', chave.cor);
        await req.query(`
          ${montarChaveItemTipada()}
      ...
      throw traduzirErroLinx(error, `Não foi possível acrescentar ${produto}${cor ? ` cor ${cor}` : ''} ao romaneio ${romaneioTrim}`);
    }
  ...
  try {
    ...
    // Os itens já entraram (e o estoque já subiu pelo trigger); o que falhou é
    // só o total do cabeçalho. A mensagem tem que dizer isso, senão o operador
    // acha que nada foi gravado e bipa tudo de novo.
    throw traduzirErroLinx(
      error,
      `Os ${qtdeAdicionada} item(ns) entraram no romaneio ${romaneioTrim}, mas o total do cabeçalho não foi atualizado`
    );
  }
...
async function executeSaidaAppendCore(pool, params) {
  ...
  // Mesma trava do lado da entrada: os triggers LXI/LXU de ESTOQUE_PROD1_SAI
  // também recusam movimento anterior ao fechamento/ajuste.
  const trava = await verificarTravaMovimento(pool, {
    tipo: 'saida',
    romaneio: romaneioTrim,
    filial: filialNome,
  ...
  for (const item of itens) {
    ...
    const cor = item.corProduto && String(item.corProduto).trim() ? String(item.corProduto).trim() : '';
    // Busca tolerante (cor '06' vs '6') que devolve a chave exata da linha — o
    // UPDATE endereça por ela, em seek, sem varrer a tabela pegando lock.
    const { tamanhoBase, chave } = await localizarItemDoRomaneio(pool, {
      tabela: 'ESTOQUE_PROD1_SAI',
      romaneio: romaneioTrim,
      filialGrav,
      produto,
      cor,
    });
    const saCol = 'SA_' + tamanhoBase;
  ...
  try {
    ...
    // Os itens já saíram (o trigger já baixou o estoque); só o total do
    // cabeçalho ficou para trás — dizer isso evita bipar tudo de novo.
    throw traduzirErroLinx(
      error,
      `Os ${qtdeAdicionada} item(ns) foram lançados no romaneio ${romaneioTrim}, mas o total do cabeçalho não foi atualizado`
    );
  }
...
/**
 * Colunas de grade (SA_n / EN_n) preenchidas na linha do item.
 *
 * A quantidade de um item vive em DUAS colunas ao mesmo tempo: `QTDE` (o total)
 * e a coluna do tamanho. Para corrigir a quantidade sem quebrar o estoque é
 * preciso saber QUAL coluna de tamanho carrega a peça — e o `TAMANHO_BASE` do
 * cadastro não responde isso sozinho, porque um romaneio digitado no Linx pode
 * ter usado outra posição da grade (ver `grade-tamanhos-posicional-linx`).
 * Então a coluna vem da própria linha: a que está preenchida.
 *
 * Devolve `colunas` com todas as preenchidas. Mais de uma = item de grade aberta
 * (P/M/G na mesma linha): aí não há como saber de qual tamanho é a divergência,
 * e quem chama recusa a edição em vez de chutar.
 */
async function lerGradeDoItem(pool, { tabela, prefixo, chave, tamanhoBase }) {
  const cols = [];
  for (let i = 1; i <= 48; i += 1) cols.push(`${prefixo}${i}`);
  ...
  const result = await req.query(`
    ${montarChaveItemTipada()}
    SELECT TOP 1 ISNULL(QTDE, 0) AS QTDE, ${cols.map((c) => `ISNULL(${c}, 0) AS ${c}`).join(', ')}
  ...
  const preenchidas = cols.filter((c) => Number(row[c]) !== 0);
  return {
    qtde: Number(row.QTDE) || 0,
    colunas: preenchidas,
    // Linha sem grade preenchida (item que já foi zerado antes): volta a usar a
    // posição do cadastro, a mesma que a criação do romaneio usa.
    coluna: preenchidas.length === 1 ? preenchidas[0] : `${prefixo}${tamanhoBase}`,
  };
}

/**
 * CORRIGIR A QUANTIDADE DE UM ITEM que já está no romaneio — o caso da
 * conferência que achou menos (ou mais) peça do que o romaneio diz.
 *
 * Quantidade ABSOLUTA, não delta: quem chama diz "este item são 2", e daqui sai
 * o movimento de estoque da diferença. Zero é válido e significa "esta peça não
 * existiu": a linha FICA no romaneio com QTDE 0 em vez de ser apagada, porque
 * DELETE dispara o trigger LXD, que reverte o estoque por conta própria e
 * duplicaria a devolução (ver `exclusao-romaneio-retorno-estoque-dobro`).
 *
 * O estoque não é mexido na mão: quem move é o trigger do Linx. `QTDE` e a
 * coluna de grade andam juntas porque o bloco "ATUALIZA ESTOQUE PA" do
 * LXU_ESTOQUE_PROD1_SAI / _ENT é disparado por UPDATE(SA_n) / UPDATE(EN_n),
 * nunca por UPDATE(QTDE) — mexer só em QTDE deixaria o estoque parado e a grade
 * ES1..ES48 mentindo.
 *
 *  - saida   → ESTOQUE_PROD1_SAI: reduzir devolve peça à ORIGEM, aumentar tira mais.
 *  - entrada → ESTOQUE_PROD1_ENT: reduzir tira peça do DESTINO, aumentar acrescenta.
 */
async function executeItemQtdeSetCore(pool, params) {
  const { tipo, romaneio, filial, produto, corProduto, qtdeNova } = params;

  if (tipo !== 'saida' && tipo !== 'entrada') {
    throw new Error("Tipo inválido: use 'saida' ou 'entrada'");
  }

  const romaneioTrim = (romaneio || '').trim();
  const produtoTrim = (produto || '').trim();
  if (!romaneioTrim) throw new Error('Romaneio é obrigatório para corrigir a quantidade');
  if (!produtoTrim) throw new Error('Produto é obrigatório para corrigir a quantidade');

  const alvo = Math.floor(Number(qtdeNova));
  if (!Number.isFinite(alvo) || alvo < 0) {
    throw new Error('Quantidade inválida: use um número inteiro maior ou igual a zero');
  }

  const filialNome = await resolverNomeFilial(pool, filial);
  if (!filialNome) throw new Error('Filial é obrigatória para corrigir a quantidade');
  const filialGrav = filialComoNoLinx(filialNome);

  const tabela = tipo === 'saida' ? 'ESTOQUE_PROD1_SAI' : 'ESTOQUE_PROD1_ENT';
  const prefixo = tipo === 'saida' ? 'SA_' : 'EN_';
  const cor = corProduto && String(corProduto).trim() ? String(corProduto).trim() : '';

  // Chave exata da linha (tolera cor '06' vs '6') para o UPDATE ir em seek.
  const { tamanhoBase, chave } = await localizarItemDoRomaneio(pool, {
    tabela,
    romaneio: romaneioTrim,
    filialGrav,
  ...
  if (!chave) {
    throw new Error(
      `${produtoTrim}${cor ? ` cor ${cor}` : ''} não está no romaneio ${romaneioTrim} de ${filialNome}.`
    );
  }

  // Mesma trava do append: só saída AVULSA é corrigível aqui. Uma saída pareada
  // com entrada/nota de transferência tem o outro lado amarrado pelo Linx —
  // mexer na quantidade daqui deixaria origem e destino divergentes, e a
  // divergência de uma transferência faturada é assunto de conferência, não de
  // correção silenciosa. Os romaneios de defeito passam: nascem avulsos.
  if (tipo === 'saida') {
    const reqPar = pool.request();
    reqPar.input('romaneio', romaneioTrim);
    reqPar.input('filial', filialGrav);
    const par = await reqPar.query(`
      SELECT
        (SELECT COUNT(*) FROM ESTOQUE_PROD_SAI WITH (NOLOCK)
          WHERE LTRIM(RTRIM(ROMANEIO_PRODUTO)) = @romaneio
            AND LTRIM(RTRIM(FILIAL)) = @filial
        ...
        (SELECT COUNT(*) FROM LOJA_SAIDAS WITH (NOLOCK)
          WHERE LTRIM(RTRIM(ROMANEIO_PRODUTO)) = @romaneio
            ...
            AND ISNULL(SAIDA_CANCELADA, 0) = 1) AS CANCELADA
    `);
    const info = par.recordset[0] || {};
    if (Number(info.CANCELADA)) {
      throw new Error(`Romaneio ${romaneioTrim} está cancelado e não pode ser corrigido.`);
    }
    if (Number(info.COM_DESTINO) || Number(info.ITENS_LOJA) || Number(info.COM_NOTA)) {
      throw new Error(
        `Romaneio ${romaneioTrim} está pareado com uma entrada/nota de transferência — só saídas avulsas podem ser corrigidas por aqui.`
      );
    }
  }

  const grade = await lerGradeDoItem(pool, { tabela, prefixo, chave, tamanhoBase });
  if (!grade) {
    throw new Error(`Item não encontrado no romaneio ${romaneioTrim} de ${filialNome}.`);
  }
  if (grade.colunas.length > 1) {
    throw new Error(
      `${produtoTrim} está lançado em mais de um tamanho na mesma linha do romaneio ${romaneioTrim} ` +
        `(${grade.colunas.join(', ')}). Corrija esse item direto no Linx: daqui não dá para saber de qual tamanho é a diferença.`
    );
  }

  const delta = alvo - grade.qtde;
  if (delta === 0) {
    return {
      success: true,
      romaneio: romaneioTrim,
      filial: filialNome,
      qtdeAnterior: grade.qtde,
      qtdeNova: alvo,
      delta: 0,
      coluna: grade.coluna,
      message: 'Quantidade já era essa — nada foi alterado.',
    };
  }

  // Trava do Linx ANTES de escrever: sem isso o trigger responde com `rollback`
  // e a tela recebe "the transaction ended in the trigger", que não diz nada.
  const trava = await verificarTravaMovimento(pool, {
    tipo,
    romaneio: romaneioTrim,
    filial: filialNome,
    itens: [{ produto: produtoTrim, corProduto: cor }],
  });
  const motivoTrava = motivoTravaMovimento(trava, romaneioTrim);
  if (motivoTrava) throw new Error(motivoTrava);
  ...
  req.input('keyProduto', chave.produto);
  req.input('keyCor', chave.cor);
  req.input('delta', delta);

  // Delta (e não valor absoluto) para não zerar o que houver em outra coluna da
  // linha, e para o trigger enxergar exatamente a diferença a movimentar. Na
  // entrada, VALOR acompanha a quantidade nova pelo custo já gravado, como no
  // append (ver montarCustoEntradaSql).
  const setValor =
    tipo === 'entrada'
      ? `,
             VALOR = CAST(ROUND((ISNULL(QTDE, 0) + @delta) * ISNULL(CUSTO1, 0), 2) AS NUMERIC(14, 2))`
      : '';
  ...
  try {
    await req.query(`
      ${montarChaveItemTipada()}
      UPDATE ${tabela}
         SET QTDE = ISNULL(QTDE, 0) + @delta,
             ${grade.coluna} = ISNULL(${grade.coluna}, 0) + @delta${setValor}
       WHERE ${MATCH_CHAVE_ITEM}
    `);
  } catch (error) {
    throw traduzirErroLinx(
      error,
      `Não foi possível corrigir ${produtoTrim}${cor ? ` cor ${cor}` : ''} no romaneio ${romaneioTrim}`
    );
  }

  // Total do cabeçalho acompanha o item. Igual ao append: mexer em QTDE_TOTAL
  // não aciona a lógica de estoque de LOJA_SAIDAS / LOJA_ENTRADAS — quem move o
  // estoque é o trigger da tabela de itens, via coluna de grade.
  const reqTotal = pool.request();
  reqTotal.input('romaneio', romaneioTrim);
  reqTotal.input('filial', filialGrav);
  reqTotal.input('delta', delta);
  const tabelaCab = tipo === 'saida' ? 'LOJA_SAIDAS' : 'LOJA_ENTRADAS';
  const setValorTotal =
    tipo === 'entrada'
      ? `,
             VALOR_TOTAL = ${montarValorTotalEntradaSql({ romaneio: '@romaneio', filial: '@filial' })}`
  ...
  try {
    await reqTotal.query(`
      UPDATE ${tabelaCab}
         SET QTDE_TOTAL = ISNULL(QTDE_TOTAL, 0) + @delta${setValorTotal}
       WHERE LTRIM(RTRIM(ROMANEIO_PRODUTO)) = @romaneio
         AND LTRIM(RTRIM(FILIAL)) = @filial
    `);
  } catch (error) {
    // O item já foi corrigido e o trigger já movimentou o estoque; o que ficou
    // para trás é só o total do cabeçalho. Dizer isso evita refazer a correção.
    throw traduzirErroLinx(
      error,
      `${produtoTrim} foi corrigido para ${alvo} no romaneio ${romaneioTrim}, mas o total do cabeçalho não foi atualizado`
    );
  }
  ...
  return {
    ...
    delta,
    coluna: grade.coluna,
    message: `${produtoTrim}: ${grade.qtde} → ${alvo} no romaneio ${romaneioTrim}.`,
  };
}
...
async function executeItemQtdeSet(pool, params) {
  return executeInTransaction(pool, (transaction) => executeItemQtdeSetCore(transaction, params));
}
...
module.exports = {
  ...
  executeSaidaAppend,
  // Correção de quantidade de item que JÁ está no romaneio (tela Defeitos).
  executeItemQtdeSet,
  // A tela consulta a trava do Linx antes de deixar bipar (ver GET das rotas
  // entrada-editar / saida-editar).
