...
async function executeEntradaLote(pool, params) {
  ...
  // NOTA: TIPO_ROMANEIO é omitido do INSERT propositalmente.
  // O trigger LXI_ESTOQUE_PROD_ENT verifica TIPO_ROMANEIO contra ESTOQUE_ROMANEIO_TIPO,
  // e 'ENTRADA AVULSA' não existe nessa tabela. Omitir a coluna faz UPDATE(TIPO_ROMANEIO)=FALSE,
  // bypassing a verificação. O campo fica NULL (2581+ registros existentes já são NULL).
  const req3 = pool.request();
  req3.input('romaneioEntrada', romaneioEntrada);
  req3.input('filial', filialEscaped);
  req3.input('dataStr', dataStr);
