      ...
      const query = `
        ...
        LEFT JOIN (
          ...
          GROUP BY PRODUTO, COR_PRODUTO
           ...
           AND (RTRIM(LTRIM(CAST(c.COR_PRODUTO AS VARCHAR(20)))) = RTRIM(LTRIM(CAST(pbMatch.COR_PRODUTO AS VARCHAR(20)))) OR TRY_CONVERT(INT, c.COR_PRODUTO) = TRY_CONVERT(INT, pbMatch.COR_PRODUTO))
