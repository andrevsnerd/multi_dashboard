...
import ClienteDetalhePage from "@/components/clientes/ClienteDetalhePage";
...
export default async function ClienteDetalhePageRoute({
  params,
  searchParams,
}: Props) {
  const { company: companySlug } = await params;
  ...
  const chave = typeof sp.chave === "string" ? sp.chave : undefined;
  if (!cliente) notFound();
  if (cliente === "SEM CADASTRO" && !chave) notFound();
