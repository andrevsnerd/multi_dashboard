...
async function fetchProducts(
  ...
  if (filterByRegistrationDate) {
    searchParams.set("filterByRegistrationDate", "true");
  }
...
export default function ProductsRecentPage({
  companyKey,
  companyName,
  ...
  const [groupByColor, setGroupByColor] = useState(true); // Sempre true por padrão
    ...
    async function load() {
      ...
      try {
        const [productsData, summaryData] = await Promise.all([
          fetchProducts(companyKey, range, selectedFilial, selectedGrupos, selectedLinhas, selectedColecoes, selectedSubgrupos, selectedGrades, groupByColor, selectedProductId, selectedProductId ? null : (searchTerm && searchTerm.trim().length >= 2 ? searchTerm.trim() : null), true), // filterByRegistrationDate = true para produtos recentes
          fetchSummary(companyKey, range, selectedFilial, selectedGrupos, selectedLinhas, selectedColecoes, selectedSubgrupos, selectedGrades, selectedProductId, selectedProductId ? null : (searchTerm && searchTerm.trim().length >= 2 ? searchTerm.trim() : null), true), // filterByRegistrationDate = true para produtos recentes
        ]);
        if (active) {
  ...
  }, [companyKey, range, rangeKey, selectedFilial, selectedGrupos, selectedLinhas, selectedColecoes, selectedSubgrupos, selectedGrades, groupByColor]);

  return (
    <div className={styles.wrapper}>
      <div className={styles.header}>
        <h1 className={styles.title}>Produtos por Cadastro</h1>
